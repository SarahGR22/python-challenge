...
great_increase = -float('inf') #decimal point numbers up to negative infinite 
great_decrease = -float('inf') #decimal point numbers up to infinite
...
with open(file_to_load) as financial_data:
    ...
    for row in reader:
        ...
        monthly_changes.append(monthly_change) #adding month change to end of month list


 # Calculate the greatest increase in profits (month and amount)
        if monthly_change > great_increase:
            great_increase = monthly_change #if if statment is true: value of greate will chnage to the value of monthly change
        ...
        average = (sum(monthly_changes)- monthly_changes[0] / 86) # average 
